  ...
  try {
    switch (action.type) {
      ...
      case "unequipItem": {
        ...
        const equipped =
          newState.equipment[slotName as keyof typeof newState.equipment];
        if (equipped) {
          // Add to inventory
          const nextSlot =
            Math.max(0, ...newState.inventory.items.map((i) => i.slotNumber!)) +
            1;
          ...
          delete newState.equipment[
            slotName as keyof typeof newState.equipment
          ];
        }
        break;
      }
      ...
      case "swapInventorySlots": {
        ...
        const item1 = newState.inventory.items.find(
          (i) => i.slotNumber === slot1
        );
        const item2 = newState.inventory.items.find(
          (i) => i.slotNumber === slot2
        );
      ...
      case "equipAbility": {
        ...
        const hotbarSlot = newState.abilities.hotbar.find(
          (h) => h.slot === slotIndex
        );
        if (hotbarSlot) {
          hotbarSlot.abilityId = abilityId;
          const learned = newState.abilities.learned.find(
